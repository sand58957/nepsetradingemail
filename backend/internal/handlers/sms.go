...
// DeleteCampaign deletes an SMS campaign (any status).
func (h *SMSHandler) DeleteCampaign(c echo.Context) error {
	...
	tx, _ := h.db.Beginx()
	tx.Exec("DELETE FROM sms_campaign_messages WHERE campaign_id = $1", id)
	result, err2 := tx.Exec("DELETE FROM sms_campaigns WHERE id = $1 AND account_id = $2", id, accountID)
	if err2 != nil {
		tx.Rollback()
