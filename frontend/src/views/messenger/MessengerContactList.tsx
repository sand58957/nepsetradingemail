...
const MessengerContactList = () => {
  ...
  const [editingContact, setEditingContact] = useState<MessengerContact | null>(null)
  const [newContact, setNewContact] = useState({ psid: '', name: '', email: '' })
  const [addingContact, setAddingContact] = useState(false)
  const [availableGroups, setAvailableGroups] = useState<MessengerContactGroup[]>([])
  ...
  const [menuContact, setMenuContact] = useState<MessengerContact | null>(null)
  ...
  const handleDelete = async () => {
    ...
    try {
      await messengerService.deleteContact(deletingId)
  ...
  const handleBulkDelete = async () => {
    ...
    try {
      ...
      for (const id of selectedIds) {
        try {
          await messengerService.deleteContact(id)
          deleted++
        } catch {
  ...
  const handleAddContact = async () => {
    if (!newContact.psid) {
      setSnackbar({ open: true, message: 'PSID is required', severity: 'error' })
    ...
    try {
      await messengerService.createContact({
        psid: newContact.psid,
  ...
  // Open edit dialog
  const openEditDialog = (contact: MessengerContact) => {
  ...
  const handleEditContact = async () => {
    ...
    try {
      await messengerService.updateContact(editingContact.id, {
        psid: editingContact.psid,
  ...
  const handleExport = async () => {
    try {
      const blob = await messengerService.exportContacts()
      const url = window.URL.createObjectURL(blob)
      const a = document.createElement('a')

      a.href = url
      a.download = 'messenger_contacts.csv'
          ...
          action={
            <div className='flex gap-2 flex-wrap'>
              ...
              <Button
                ...
                onClick={() => router.push(`/${locale}/messenger/contacts/import`)}
              >
                Import CSV
              </Button>
              <Button
          ...
          <Grid container spacing={4} className='mb-4'>
            <Grid size={{ xs: 12, sm: 6, md: 4 }}>
              <TextField
                fullWidth
                size='small'
                placeholder='Search by PSID, name, or email...'
                value={searchTerm}
                    ...
                    <TableRow key={contact.id} hover selected={selectedIds.includes(contact.id)}>
                      ...
                      <TableCell>
                        <Box className='flex items-center gap-2'>
                          {contact.profile_pic ? (
                            <Avatar src={contact.profile_pic} alt={contact.name} sx={{ width: 28, height: 28 }} />
                          ) : (
        ...
        <DialogContent>
          <div className='flex flex-col gap-4 mt-2'>
            <TextField
              ...
              value={newContact.psid}
              onChange={e => setNewContact({ ...newContact, psid: e.target.value })}
              helperText='The Facebook Page-Scoped ID for this user'
            />
            <TextField
              fullWidth
              label='Name'
          ...
          {editingContact && (
            <div className='flex flex-col gap-4 mt-2'>
              <TextField
                fullWidth
                label='PSID *'
                value={editingContact.psid}
                onChange={e => setEditingContact({ ...editingContact, psid: e.target.value })}
              />
              <TextField
                fullWidth
                label='Name'
...
export default MessengerContactList
