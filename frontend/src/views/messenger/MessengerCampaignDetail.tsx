...
// Type Imports
import type { MessengerCampaign, MessengerCampaignRecipient } from '@/types/messenger'
...
interface MessengerCampaignDetailProps {
  id: string
}

const MessengerCampaignDetail = ({ id }: MessengerCampaignDetailProps) => {
  const router = useRouter()
  const { lang } = useParams()
  const locale = (lang as string) || 'en'

  const [campaign, setCampaign] = useState<MessengerCampaign | null>(null)
  const [statusBreakdown, setStatusBreakdown] = useState<{ status: string; count: number }[]>([])
  const [recipients, setRecipients] = useState<MessengerCampaignRecipient[]>([])
  ...
  useEffect(() => {
    const fetchCampaign = async () => {
      ...
      try {
        const response = await messengerService.getCampaign(Number(id))
  ...
  const handleTestSend = async () => {
    ...
    try {
      const response = await messengerService.testCampaign(Number(id), testPsid)
  ...
  const handleSendCampaign = async () => {
    ...
    try {
      const response = await messengerService.sendCampaign(Number(id))
      ...
      // Refresh data
      const updated = await messengerService.getCampaign(Number(id))
  ...
  const handlePause = async () => {
    try {
      await messengerService.pauseCampaign(Number(id))
  ...
  const handleResume = async () => {
    try {
      await messengerService.resumeCampaign(Number(id))
                ...
                >
                  {campaign.image_url && (
                    <Box className='mb-2 rounded overflow-hidden' sx={{ maxWidth: 300 }}>
                      <img src={campaign.image_url} alt='Campaign media' style={{ width: '100%', display: 'block' }} onError={(e) => { (e.target as HTMLImageElement).style.display = 'none' }} />
                    </Box>
                  )}
                    ...
                    <TableBody>
                      {recipients.map(recipient => (
                        <TableRow key={recipient.id}>
                          ...
                          <TableCell>
                            <Typography variant='body2'>{recipient.psid}</Typography>
                          </TableCell>
                          <TableCell>
                            <Chip
      ...
      <Dialog open={testDialogOpen} onClose={() => setTestDialogOpen(false)} maxWidth='sm' fullWidth>
        ...
        <DialogContent>
          ...
          <TextField
            ...
            helperText='Enter the Page-Scoped ID (PSID) to send the test to'
            className='mt-2'
            type='text'
          />
        </DialogContent>
      ...
      <Dialog open={sendDialogOpen} onClose={() => setSendDialogOpen(false)}>
        ...
        <DialogContent>
          <Alert severity='warning' className='mb-3'>
            This will send Messenger messages to all matching opted-in contacts. This action cannot be undone.
          </Alert>
          <Typography>
...
export default MessengerCampaignDetail
