...
// Type Imports
import type { MessengerContactGroupWithCount } from '@/types/messenger'

const MessengerCreateCampaign = () => {
  ...
  const [availableGroups, setAvailableGroups] = useState<MessengerContactGroupWithCount[]>([])
  ...
  useEffect(() => {
    const fetchTags = async () => {
      try {
        const response = await messengerService.getContactTags()
  ...
  useEffect(() => {
    const fetchAudienceCount = async () => {
      ...
      try {
        ...
        const response = await messengerService.getAudienceCount(filter)
  ...
  const handleCreate = async () => {
    ...
    try {
      ...
      const response = await messengerService.createCampaign(payload)

      setSnackbar({ open: true, message: 'Campaign created', severity: 'success' })

      // Navigate to campaign detail
      setTimeout(() => {
        router.push(`/${locale}/messenger/campaigns/${response.data.id}`)
      }, 500)
    } catch {
  ...
  const handleConfirmSendNow = async () => {
    ...
    try {
      ...
      // Navigate to campaign detail
      setTimeout(() => {
        router.push(`/${locale}/messenger/campaigns/${campaignId}`)
      }, 1000)
    } catch {
      setSnackbar({ open: true, message: 'Failed to create campaign', severity: 'error' })
    } finally {
  ...
  const handleTestSend = async () => {
    if (!testPsid.trim()) {
      setSnackbar({ open: true, message: 'Please enter a PSID', severity: 'error' })
    ...
    try {
      ...
      const testResponse = await messengerService.testCampaign(campaignId, testPsid.trim())
          ...
          <Card>
            <CardContent>
              <div className='flex items-center justify-between flex-wrap gap-4'>
                <div>
                  <Typography variant='h5'>Create Messenger Campaign</Typography>
                  <Typography variant='body2' color='text.secondary'>
                    Send a message to your Messenger contacts via your Facebook Page
                  </Typography>
                </div>
                <Button
                  variant='outlined'
              ...
              <div className='flex flex-col gap-5'>
                ...
                <div>
                  <TextField
                    ...
                    rows={6}
                    label='Message Text *'
                    placeholder='Type your Messenger message here...'
                    value={messageText}
                    onChange={e => setMessageText(e.target.value)}
                    helperText={
                      <span style={{ display: 'flex', justifyContent: 'space-between', width: '100%' }}>
                        <span>Plain text message sent to recipients</span>
                ...
                <TextField
                  ...
                  helperText='Optional image to send with the message (JPEG, PNG, GIF)'
                />
                ...
                {messageText.length > 0 && (
                  ...
                  >
                    <div className='flex items-center gap-2 mb-2'>
                      <i className='tabler-brand-facebook' style={{ fontSize: 20, color: '#1877F2' }} />
            ...
            <CardContent>
              <div className='flex flex-col gap-3'>
                ...
                <div className='flex gap-2'>
                  ...
                  <Typography variant='body2'>Optionally attach an image URL to include with your message</Typography>
                </div>
                <div className='flex gap-2'>
                ...
                <Alert severity='success' className='mt-2'>
                  Messenger messaging is free -- no credits or per-message charges!
                </Alert>
              </div>
            </CardContent>
          </Card>
      ...
      <Dialog open={testDialogOpen} onClose={() => setTestDialogOpen(false)} maxWidth='sm' fullWidth>
        ...
        <DialogContent>
          <Typography variant='body2' color='text.secondary' className='mb-4'>
            Send a test message to a single Messenger user to preview before sending to all contacts.
          </Typography>
          <TextField
            fullWidth
            label='PSID (Page-Scoped ID)'
            placeholder='e.g. 1234567890123456'
            value={testPsid}
            onChange={e => setTestPsid(e.target.value)}
            helperText='Enter the Page-Scoped ID of the Messenger user to send the test to'
            className='mt-2'
          />
          {messageText && (
      ...
      <Dialog open={confirmSendNow} onClose={() => setConfirmSendNow(false)}>
        ...
        <DialogContent>
          <div className='flex flex-col gap-3 mt-1'>
            ...
            <Alert severity='info'>
              Messenger messages will be sent immediately. Make sure your message and audience are correct.
            </Alert>
          </div>
        </DialogContent>
        <DialogActions>
...
export default MessengerCreateCampaign
