...
const COLOR_OPTIONS = ['#0084FF', '#388e3c', '#d32f2f', '#f57c00', '#7b1fa2', '#0097a7', '#455a64', '#c2185b']

const MessengerGroupList = () => {
  const router = useRouter()
  const { lang } = useParams()
  const locale = (lang as string) || 'en'

  const [groups, setGroups] = useState<MessengerContactGroupWithCount[]>([])
  ...
  const [formColor, setFormColor] = useState('#0084FF')
  ...
  const [menuGroup, setMenuGroup] = useState<MessengerContactGroupWithCount | null>(null)
  ...
  const handleSave = async () => {
    ...
    try {
      if (editingId) {
        await messengerService.updateGroup(editingId, { name: formName.trim(), description: formDescription, color: formColor })
        setSnackbar({ open: true, message: 'Group updated', severity: 'success' })
      } else {
        await messengerService.createGroup({ name: formName.trim(), description: formDescription, color: formColor })
  ...
  const handleDelete = async () => {
    ...
    try {
      await messengerService.deleteGroup(deletingId)
  ...
  const openEdit = (group: MessengerContactGroupWithCount) => {
    ...
    <>
      <Card>
        <CardHeader
          ...
          subheader='Organize your Messenger contacts into groups for targeted campaigns'
          action={
              ...
              <TableBody>
                {groups.map(group => (
                  <TableRow
                    ...
                    hover
                    sx={{ cursor: 'pointer' }}
                    onClick={() => router.push(`/${locale}/messenger/groups/${group.id}`)}
                  >
                    <TableCell>
      ...
      <Menu anchorEl={anchorEl} open={Boolean(anchorEl)} onClose={() => setAnchorEl(null)}>
        <MenuItem onClick={() => {
          setAnchorEl(null)
          if (menuGroup) router.push(`/${locale}/messenger/groups/${menuGroup.id}`)
        }}>
...
export default MessengerGroupList
