...
import type { SMSContactGroup, SMSContact } from '@/types/sms'

interface SMSGroupDetailProps {
  groupId: number
}

const SMSGroupDetail = ({ groupId }: SMSGroupDetailProps) => {
  const router = useRouter()
  const { lang } = useParams()
  const locale = (lang as string) || 'en'

  // Group info
  const [group, setGroup] = useState<SMSContactGroup | null>(null)
  const [memberCount, setMemberCount] = useState(0)
  const [loadingGroup, setLoadingGroup] = useState(true)

  // Members list
  const [members, setMembers] = useState<SMSContact[]>([])
  ...
  const [allContacts, setAllContacts] = useState<SMSContact[]>([])
        ...
        <CardContent>
          <div className='flex items-center justify-between flex-wrap gap-4'>
            ...
            <Button
              ...
              onClick={() => router.push(`/${locale}/sms/groups`)}
            >
              Back to Groups
            </Button>
          </div>
