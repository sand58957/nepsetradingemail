...
const SMSCampaignList = () => {
  ...
  const [menuCampaign, setMenuCampaign] = useState<SMSCampaign | null>(null)
  ...
  const fetchCampaigns = useCallback(async () => {
    ...
    try {
      const response = await smsService.getCampaigns({
        page: page + 1,
  ...
  const handleDelete = async () => {
    ...
    try {
      await smsService.deleteCampaign(deletingId)
