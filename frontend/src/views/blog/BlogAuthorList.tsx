    ...
    <>
      <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', mb: 4 }}>
        ...
        <Button
          ...
          onClick={() => router.push(`/${locale}/blog/authors/create`)}
        >
          Add Author
        </Button>
      </Box>
        ...
        <Grid container spacing={4}>
          {authors.map(author => (
            <Grid key={author.id} size={{ xs: 12, sm: 6, md: 4 }}>
              <Card
                sx={{ height: '100%', cursor: 'pointer', '&:hover': { boxShadow: 6 }, transition: 'box-shadow 0.2s' }}
                ...
                >
                  ...
                  <Box sx={{ display: 'flex', gap: 2, mt: 1 }}>
                    <Typography variant='body2'>
                      <strong>{author.post_count || 0}</strong> posts
                    </Typography>
                    <Chip
                      label={author.is_active ? 'Active' : 'Inactive'}
                      size='small'
                      color={author.is_active ? 'success' : 'default'}
                      variant='tonal'
                    />
                  </Box>
                  <Box sx={{ mt: 1 }}>
                    <IconButton
                      ...
                      onClick={e => {
                        e.stopPropagation()
                        router.push(`/${locale}/blog/authors/${author.id}`)
                      }}
                    >
      ...
      <Dialog open={deleteDialogOpen} onClose={() => setDeleteDialogOpen(false)}>
        ...
        <DialogContent>
          <Typography>Are you sure? Posts by this author will remain but lose their author.</Typography>
        </DialogContent>
        <DialogActions>
