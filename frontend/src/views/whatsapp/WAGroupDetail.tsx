...
import type { WAContactGroup, WAContact } from '@/types/whatsapp'

interface WAGroupDetailProps {
  groupId: number
}

const WAGroupDetail = ({ groupId }: WAGroupDetailProps) => {
  const router = useRouter()
  const { lang } = useParams()
  const locale = (lang as string) || 'en'

  const [group, setGroup] = useState<WAContactGroup | null>(null)
  const [memberCount, setMemberCount] = useState(0)
  const [loadingGroup, setLoadingGroup] = useState(true)

  const [members, setMembers] = useState<WAContact[]>([])
