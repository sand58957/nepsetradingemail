...
import type { TelegramContactGroup, TelegramContact } from '@/types/telegram'

export interface TelegramGroupDetailProps {
  id: string
}

const TelegramGroupDetail = ({ id }: TelegramGroupDetailProps) => {
  ...
  // Group info
  const [group, setGroup] = useState<TelegramContactGroup | null>(null)
  ...
  const [allContacts, setAllContacts] = useState<TelegramContact[]>([])
  ...
  useEffect(() => {
    const fetchGroup = async () => {
      ...
      try {
        const response = await telegramService.getGroup(groupId)
  ...
  const fetchMembers = useCallback(async () => {
    ...
    try {
      const response = await telegramService.getGroupMembers(groupId, {
        page: page + 1,
  ...
  const fetchAllContacts = useCallback(async () => {
    ...
    try {
      const response = await telegramService.getContacts({
        page: addPage + 1,
        per_page: 25,
  ...
  const handleAddMembers = async () => {
    ...
    try {
      const response = await telegramService.addGroupMembers(groupId, addSelectedIds)
      ...
      fetchMembers()
      // Update member count
      const groupRes = await telegramService.getGroup(groupId)
  ...
  const handleRemoveMembers = async () => {
    ...
    try {
      const response = await telegramService.removeGroupMembers(groupId, selectedIds)
  ...
  const getDisplayName = (contact: TelegramContact) => {
    if (contact.first_name || contact.last_name) {
      return `${contact.first_name || ''} ${contact.last_name || ''}`.trim()
    }

    return contact.username ? `@${contact.username}` : '-'
  }
        ...
        <CardContent>
          <div className='flex items-center justify-between flex-wrap gap-4'>
            ...
            <Button
              ...
              onClick={() => router.push(`/${locale}/telegram/groups`)}
            >
              Back to Groups
            </Button>
          </div>
                ...
                <TableBody>
                  {members.map(contact => (
                    <TableRow key={contact.id} hover selected={selectedIds.includes(contact.id)}>
                      ...
                      <TableCell><Typography className='font-medium'>{contact.chat_id}</Typography></TableCell>
                      <TableCell><Typography>{contact.username ? `@${contact.username}` : '-'}</Typography></TableCell>
                  ...
                  <TableBody>
                    {allContacts.map(contact => (
                      <TableRow key={contact.id} hover>
                        ...
                        <TableCell>{contact.chat_id}</TableCell>
...
export default TelegramGroupDetail
