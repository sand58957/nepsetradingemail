...
// Type Imports
import type { TelegramCampaign } from '@/types/telegram'
...
const TelegramCampaignList = () => {
  ...
  const [menuCampaign, setMenuCampaign] = useState<TelegramCampaign | null>(null)
  ...
  const fetchCampaigns = useCallback(async () => {
    ...
    try {
      const response = await telegramService.getCampaigns({
        page: page + 1,
  ...
  const handleDelete = async () => {
    ...
    try {
      await telegramService.deleteCampaign(deletingId)
  ...
  const getDeliveryRate = (campaign: TelegramCampaign) => {
    ...
    <>
        ...
        {loading ? (
          ...
          <CardContent>
            <Typography color='text.secondary' align='center' className='py-8'>
              {globalFilter || statusFilter ? 'No campaigns match your filters' : 'No campaigns yet. Create your first Telegram campaign.'}
            </Typography>
          </CardContent>
        ) : (
          <>
      ...
      >
        <MenuItem onClick={() => {
          ...
          if (menuCampaign) {
            router.push(`/${locale}/telegram/campaigns/${menuCampaign.id}`)
          }
        }}>
...
export default TelegramCampaignList
