...
// Type Imports
import type { TelegramContact, TelegramContactGroup } from '@/types/telegram'

const TelegramContactList = () => {
  ...
  const [newContact, setNewContact] = useState({ chat_id: '', username: '', first_name: '', last_name: '' })
  ...
  const handleDelete = async () => {
    ...
    try {
      await telegramService.deleteContact(deletingId)
  ...
  const handleBulkDelete = async () => {
    ...
    try {
      ...
      for (const id of selectedIds) {
        try {
          await telegramService.deleteContact(id)
          deleted++
        } catch {
  ...
  const handleAddContact = async () => {
    if (!newContact.chat_id) {
      setSnackbar({ open: true, message: 'Chat ID is required', severity: 'error' })
  ...
  const handleEditContact = async () => {
    ...
    try {
      await telegramService.updateContact(editingContact.id, {
  ...
  const handleExport = async () => {
    try {
      const blob = await telegramService.exportContacts()
      const url = window.URL.createObjectURL(blob)
      const a = document.createElement('a')

      a.href = url
      a.download = 'telegram_contacts.csv'
          ...
          <Grid container spacing={4} className='mb-4'>
            <Grid size={{ xs: 12, sm: 6, md: 4 }}>
              <TextField
                fullWidth
                size='small'
                placeholder='Search by chat ID, username, or name...'
                value={searchTerm}
                ...
                <TableBody>
                  {contacts.map(contact => (
                    <TableRow key={contact.id} hover selected={selectedIds.includes(contact.id)}>
                      ...
                      <TableCell>
                        <Typography className='font-medium'>{contact.chat_id}</Typography>
                      </TableCell>
                      <TableCell>
                        <Typography>{contact.username ? `@${contact.username}` : '-'}</Typography>
                      </TableCell>
                      <TableCell>
        ...
        <DialogContent>
          <div className='flex flex-col gap-4 mt-2'>
            <TextField
              ...
              value={newContact.chat_id}
              onChange={e => setNewContact({ ...newContact, chat_id: e.target.value })}
              helperText='The Telegram chat ID (numeric)'
              type='number'
            />
            <TextField
              fullWidth
          ...
          {editingContact && (
            <div className='flex flex-col gap-4 mt-2'>
              <TextField
                fullWidth
                label='Chat ID *'
                value={editingContact.chat_id}
                onChange={e => setEditingContact({ ...editingContact, chat_id: Number(e.target.value) })}
                type='number'
              />
              <TextField
                fullWidth
