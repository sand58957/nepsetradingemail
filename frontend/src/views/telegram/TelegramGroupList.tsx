...
const COLOR_OPTIONS = ['#0088cc', '#388e3c', '#d32f2f', '#f57c00', '#7b1fa2', '#0097a7', '#455a64', '#c2185b']
...
const TelegramGroupList = () => {
  ...
  const [groups, setGroups] = useState<TelegramContactGroupWithCount[]>([])
  ...
  const [formColor, setFormColor] = useState('#0088cc')
  ...
  const openEdit = (group: TelegramContactGroupWithCount) => {
    ...
    <>
      <Card>
        <CardHeader
          ...
          subheader='Organize your Telegram contacts into groups for targeted campaigns'
          action={
              ...
              <TableBody>
                {groups.map(group => (
                  <TableRow
                    ...
                    hover
                    sx={{ cursor: 'pointer' }}
                    onClick={() => router.push(`/${locale}/telegram/groups/${group.id}`)}
                  >
                    <TableCell>
